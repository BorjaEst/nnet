...
-spec transaction(Network, Function) -> {atomic, ok} when
    Network  :: network:id(),
    Function :: function().
transaction(NNET_id, Function) -> 
    mnesia:transaction(
        fun() -> 
            [NNET_0] = mnesia:read(network, NNET_id),
...
%% @end
%%-------------------------------------------------------------------
-spec reinitialise_bias(NNET, N) -> ok when 
    NNET :: network:network(),
    N   :: neuron:id().
reinitialise_bias(NNET, N_id) -> 
...
%% @end
%%-------------------------------------------------------------------
-spec switch_activation(NNET, N, Function_name) -> ok when 
    NNET :: network:network(),
    N   :: neuron:id(),
    Function_name :: activation:func().
switch_activation(NNET, N_id, Func) -> 
    [N] = mnesia:read(neuron, N_id),
...
%% @end
%%-------------------------------------------------------------------
-spec reinitialise_weights(NNET, N) -> ok when 
    NNET :: network:network(),
    N   :: neuron:id().
reinitialise_weights(NNET, N_id) -> 
    Links = network:in_links(NNET, N_id),
...
-spec reinitialise_weights(Network, N, Percentage) -> ok when 
    ...
    N          :: neuron:id(),
    Percentage :: float().
reinitialise_weights(NNET, N_id, Percentage) -> 
    Links = ltools:rand(network:in_links(NNET, N_id), Percentage),
    [link:delete(Link) || Link <- Links],
    NNET.
...
%% @end
%%-------------------------------------------------------------------
-spec divide_neuron(NNET, N) -> ok when 
    NNET :: network:network(),
    N   :: neuron:id().
divide_neuron(NNET, N1_id) -> 
    [N1] = mnesia:read(neuron, N1_id),
     N2  = neuron:clone(N1),
     ok  = mnesia:write(N2),
    Map = #{N1_id => neuron:id(N2)},
    OutMove = ltools:rand(network:out_links(NNET, N1_id), 0.5),
    [ok = link:move(Link, Map) || Link <- OutMove],
    InMove  = ltools:rand(network:in_links( NNET, N1_id), 0.5),
    [ok = link:move(Link, Map) || Link <- InMove ],
    network:move_links(
        network:add_neuron(NNET, neuron:id(N2)),
...
%% @end
%%-------------------------------------------------------------------
-spec merge_neurons(NNET, N1, N2) -> ok when 
    NNET :: network:network(),
    N1  :: neuron:id(),
    N2  :: neuron:id().
merge_neurons(NNET, N1_id, N2_id) -> 
    Map = #{N1_id => N2_id},
    [link:merge(Link, Map) || Link <- network:out_links(NNET, N1_id)],
    [link:merge(Link, Map) || Link <- network:in_links( NNET, N1_id)],
    ok = mnesia:delete(neuron, N1_id, write),
    network:del_neuron( 
        network:move_links(NNET, network:links(NNET, N1_id), Map),
        N1_id).
...
%% @end
%%-------------------------------------------------------------------
-spec connect_all(NNET, From, To) -> ok when 
    NNET  :: network:network(),
    From :: [neuron:id()],
    To   :: [neuron:id()].
connect_all(NNET, From, To) -> 
    network:add_links(NNET, [{N1,N2} || N1 <- From, N2 <- To]).
...
%% @end
%%-------------------------------------------------------------------
-spec connect_allowed(NNET, From, To) -> ok when 
    NNET  :: network:network(),
    From :: [neuron:id()],
    To   :: [neuron:id()].
connect_allowed(NNET, From, To) -> 
    connect_allowed(NNET, [{N1,N2} || N1 <- From, N2 <- To]).

connect_allowed(NNET_0, [L|Lx]) -> 
    try network:add_link(NNET_0, L) of 
          NNET_1               -> connect_allowed(NNET_1, Lx)
    catch error:{bad_link, _} -> connect_allowed(NNET_0, Lx)
    end;
...
%% @end
%%-------------------------------------------------------------------
-spec disconnect_all(NNET, From, To) -> ok when 
    NNET  :: network:network(),
    From :: [neuron:id()],
    To   :: [neuron:id()].
disconnect_all(NNET, From, To) -> 
    network:del_links(NNET, [{N1,N2} || N1 <- From, N2 <- To]).
...
%% @end
%%-------------------------------------------------------------------
increase_connections(NNET) ->
    #{size:=Size} = network:info(NNET),
    Size_factor   = math:sqrt(Size),
    From = ltools:rand(network:neurons(NNET), 1/Size_factor),
    To   = ltools:rand(network:neurons(NNET), 1/Size_factor),
    connect_allowed(NNET, From, To).
...
%% @end
%%-------------------------------------------------------------------
reduce_connections(NNET, From, To) -> 
    #{size:=Size} = network:info(NNET),
    Size_factor   = math:sqrt(Size),
    From = ltools:rand(network:neurons(NNET), 1/Size_factor),
    To   = ltools:rand(network:neurons(NNET), 1/Size_factor),
